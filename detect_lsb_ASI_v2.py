...
input_privious_asi = f"{file_name2}.pdf"

output = f"{file_name}_result.pdf"
output2 = f"{file_name2}_result.pdf"
...
class StampLibrary:
    ...
    def _clone_keep_size(
        ...
        # write comment text into /Contents
        if comment is not None:
            obj = _upsert_pdf_key(obj, "/Contents", f"({_pdf_escape_paren(comment)})")
        ...
        for det in detections_pp[i]:
            ...
            try:
                lib[label].clone_at(
                    ...
                    y=rect.y0,
                    anchor="tl",
                    comment=f"confidence={det['confidence']:.3f}",
                )
                # lib[label].clone(page, rect)
            except KeyError:
                # fallback: keep your old rectangle behavior if no stamp template exists
                annot = page.add_rect_annot(rect)
                annot.set_info(
                    subject=label, content=f"confidence={det['confidence']:.3f}"
                )
                annot.set_colors(stroke=get_color_for_label(label))
                annot.set_border(width=1)
                annot.update()

    for i in range(len(detections_pp_privious)):
        page = doc[i + PAGE_OFFSET]  # original page i
        ...
        for det in detections_pp_privious[i]:
            label = str(det["label"])
            objects[label] = objects.get(label, 0) + 1
            ...
            try:
                ...
                annot.set_info(
                    subject=label + " old",
                    content=f"confidence={det['confidence']:.3f}",
                )
                annot.set_colors(stroke=get_color_for_label(label))
                annot.set_border(width=1)
                annot.update()
...
def detect_result_with_cloud(img, model, results):
    ...
    for result in results:
        ...
        for r in model.predict(
            ...
            for box in r.boxes:
                ...
                # if r.names[int(box.cls)] == "wall AV combo 75cd":
                #     cv2.imwrite(
                #         "detect_images/lsb_fa_test/lsb_fa_wall_av"
                #         + str(total_changed)
                #         + ".png",
                #         patch,
                #     )
    ...
    # 转换为 Tensor
    boxes = torch.tensor([[d["x1"], d["y1"], d["x2"], d["y2"]] for d in dets])
    scores = torch.tensor([d["confidence"] for d in dets])  # 用加权分数做排序
    # labels = [d["label"] for d in dets]  # 可用于扩展类别感知 NMS
...
def run(pdf_path, input_privious_asi, out_path, dpi, Yolo=True):
    ...
    doc = fitz.open(pdf_path)
    doc_privious_ais = fitz.open(input_privious_asi)
    ...
    for i, page in enumerate(doc):
        ...
        if pix.n == 4:
            ...
            img_privious_asi = cv2.cvtColor(img_privious_asi, cv2.COLOR_RGBA2RGB)

        if Yolo:
            dets_result = detect_page_B_with_weighted_nms(img, MODEL)
            dets = detect_result_with_cloud(img, MODEL2, dets_result)
            dets_privious_asi = detect_result_with_cloud(
                img_privious_asi, MODEL2, dets_result
            )

            dets_privious_asi.extend(dets_result)
            with open("detectResult.json", "w") as f:
                json.dump(dets, f, indent=4)
        else:
            dets = load_dets_from_json("detectResult.json")
        det_pp.append(dets)
        det_pp_privious_asi.append(dets_privious_asi)
